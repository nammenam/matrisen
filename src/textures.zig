...
const check_vk = @import("vkInit.zig").check_vk;
...
pub fn load_image_from_file(engine: *Engine, filepath: []const u8) !Engine.AllocatedImage {
    ...
    const staging_buffer = engine.create_buffer(
        image_size, c.VK_BUFFER_USAGE_TRANSFER_SRC_BIT, c.VMA_MEMORY_USAGE_CPU_ONLY);
    ...
    const img_info = std.mem.zeroInit(c.VkImageCreateInfo, .{
